...
fn absent_cache_write_field_maps_to_none() {
    ...
    assert_eq!(row.input_tokens, 14575);
    assert_eq!(row.cache_read_tokens, Some(4480));
    assert_eq!(row.output_tokens, 6);
}

#[test]
fn input_tokens_are_not_reduced_by_cached_tokens() {
    // Codex treats cached_input_tokens as a subset of input_tokens; forged
    // stores the raw totals and interprets downstream.
    let capture = CodexDriver
...
fn duplicate_turn_completed_events_keep_the_last() {
    ...
    assert_eq!(capture.rows.len(), 1);
    assert_eq!(capture.rows[0].input_tokens, 9);
