...
impl Drop for DesiredControlGuard {
    fn drop(&mut self) {
        let _ =
            self.ledger
                .release_desired_claim(DesiredSubjectKind::Epic, &self.epic, &self.token);
    }
}
...
async fn acquire_loop_control(
    ...
    let token = format!("epic-control:{}", uuid::Uuid::now_v7());
    let now = crate::config::now_iso();
    let lease = super::supervise::deadline_after(&now, 60)?;
    let id = epic.to_owned();
    let claim_token = token.clone();
    let claimed = on_ledger(&ctx.ledger, move |ledger| {
        ledger.claim_desired_control(DesiredSubjectKind::Epic, &id, &claim_token, &now, &lease)
    })
    .await?;
    if claimed.is_none() {
        return Err(Failure::refused(
...
async fn status_json(ctx: &Ctx, view: EpicView) -> Result<Value, Failure> {
    ...
    .await;
    let (live, live_legacy_non_parent, live_error) =
        match super::workstore::epic_children_with_legacy(&ctx.ledger, &view.config.epic_id).await {
            Ok((live, legacy)) => (live, legacy, None),
            Err(error) => (Vec::new(), BTreeSet::new(), Some(error.to_string())),
        };
            ...
            Ok(ready) => {
                ...
                let frontier = view
                    ...
                    .filter_map(|child| {
                        ready
                            .get(&child.id)
                            .map(|priority| json!({"childId": child.id, "priority": priority}))
                    })
                    .collect::<Vec<_>>();
                (frontier, None)
            }
    ...
    frontier.sort_by(|left, right| {
        left.get("priority")
            ...
            .then_with(|| {
                left.get("childId")
                    .and_then(Value::as_str)
                    .cmp(&right.get("childId").and_then(Value::as_str))
            })
    });
    let runs = snapshot
        .runs
...
async fn dispatch_loop_child(
    ...
    let started = response(super::ops::dispatch_frontier_run(ctx, &mut request, compiled).await)?;
    let event = json!({
        "childId": child.id,
        "runId": run_id,
        "wave": Value::Null,
...
async fn start_planning(
    ...
    let checkpoint = match effects::capture_planning_checkpoint(ctx, view, child).await {
    ...
    let ensured = if authorize {
        effects::dispatch_planning_run(ctx, config, child, &state).await?
    } else {
...
async fn advance_loop_assurance(ctx: &Ctx, view: &EpicView) -> Result<Step, Failure> {
    let pr = view
        ...
        .ok_or_else(|| Failure::internal("assurance advance has no draft PR"))?;
    let Some(state) = view.assurance.as_ref() else {
        return effects::dispatch_assurance(ctx, view, pr).await;
    };
    let started = match effects::dispatch_assurance_run(ctx, &view.config, state).await {
        Ok(started) => started,
        Err(error) => {
            let stopped = require_input_with_evidence(
                ctx,
...
pub(super) async fn advance_loop_once(ctx: &Ctx, epic: &str) -> Result<Step, Failure> {
    ...
    if view.pr.is_some() {
        return advance_loop_assurance(ctx, &view).await;
    }
    ...
    clean_children.sort_by(|left, right| left.0.cmp(&right.0));
    if let Some((child_id, run, evidence)) = clean_children.into_iter().next() {
        let child = view
            .config
            .children
            .iter()
    ...
    if let Some(step) = reconcile_loop_planning(ctx, &view).await? {
        return Ok(step);
    }
    ...
    let ready = super::workstore::ready_frozen_epic_children(&ctx.ledger, epic, &legacy_non_parent)
        .await?
        .into_iter()
        .map(|issue| (issue.id.clone(), issue))
        .collect::<BTreeMap<_, _>>();
    ...
    if frontier.is_empty() && nonterminal == 0 {
        if let Some(step) = planning_in_loop_round(ctx, &view, &statuses).await? {
            return Ok(step);
        }
    }
    ...
    if !launchable.is_empty() {
        ...
        for child in launchable {
            ...
            let generation = view
                ...
                .saturating_add(1);
            launched.push(
                dispatch_loop_child(ctx, &view.config, child, generation, compiled.clone()).await?,
            );
        }
