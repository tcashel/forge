...
rewritten to shape the LEDGER (`ensure_work_item`, `set_bead_field`,
`seed_epic`, `seed_frontier`, `set_assignee`, `set_lease_unexpired`,
`bead_revision`, `assignee` — see crates/forged/tests/support/mod.rs). Tests
...
1. **Revisions are integers** minted ONLY by spec-field writes
   ...
   drop the assertion. `env.bead_revision()` now returns the integer as a
   string. Pinning an exact revision value (`set_bead_field(_, "revision",
...
2. **No transport failures / outages.** The store is in-process.
   ...
   path. A test named "...when beads is unavailable" should become
...
6. **Refusal vocabulary preserved**: "issue not claimable: status <s>" for
   mechanism refusals; lease-held refusals are BeadLeaseHeld naming the
...
7. **seed_epic children**: readiness is now a query (open + unassigned + no
   ...
   being ready must make that true structurally: `set_bead_field(child,
   "status", "blocked")` or add a blocks edge via `set_bead_field(child,
...
- The operator-adjudicated behaviors: blocked-residue retake, closed-bead
