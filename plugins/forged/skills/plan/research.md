...
Research only enough to make the native Bead executable and falsifiable.
...
6. For an existing Bead, verify its native fields and current repository or PR
   state before proposing changes. Preserve valid history and dependencies.

Read-only discovery does not authorize dependency installation, generated file
updates, network writes, Bead mutation, or repository cleanup. Planning writes
only the operator-scoped Beads records after direction is settled.
