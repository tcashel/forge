...
2. **The agent owns turn-by-turn context (Option B); Forge owns the conversation graph.** Continuity is the agent's native session (`--session-id`/`--resume`), not a Forge-reconstructed prompt. Forge relocates the agent's transcript into its own tree and treats that native JSONL as the verbose audit log. Forge's durable layer owns only the *graph*: which conversations exist, their session pointers, and their many-to-many links to plans and to each other (the substrate for interconnected specs). Forge does **not** re-store turn content.
...
- Relocating transcripts via `CLAUDE_CONFIG_DIR` moves *all* of that config dir's contents (settings, credentials, plugins), not just transcripts — the adapter must set it deliberately per-spawn, not globally, to avoid disturbing the user's own `claude` setup.
...
- Conversations become durable, listable, cross-linkable entities — replacing today's promote-or-reap ephemeral draft model (`NewSpecModal` deletes unpromoted drafts on close).
